...
window.onload = function () {
    ...
    $("#save_user_info").click(function(){
        var head_url = $("#head_img").attr("src");
        var nick_name = $("#user_name").val();
        var descript = $("#descript").val();
        var id = JSON.parse($.cookie("user_info")).id;

        $.post("/user/update",{id:id,head_url:head_url,nick_name:nick_name,descript:descript},function(data){
            if(data.status==0){
                alert("修改成功!");
